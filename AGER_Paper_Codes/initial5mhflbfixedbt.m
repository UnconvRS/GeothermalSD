function [description, options, state0, model, schedule, plotOptions] = initial5mhflbfixedbt(varargin)
...
xmax=physdim(1:2);
sdfScaled2D= cellfun(@(x) ((x+200)./xmaxFS(1:2)), data.SDF, 'UniformOutput', false); % Scaled SDF in 2D
...
%     perc=62.5; %50%
%     perc=93.75; %75%
    wyloc=(startY+EndY)/2+200;
    CordInj=[x1+200,wyloc,250-perc;
        x2+200,wyloc,250-perc;
        x3+200,wyloc,250-perc;
        x4+200,wyloc,250-perc;
        x5+200,wyloc,250-perc]; 
    inj=[];
...
%     G.cells.volumes(fracture_cells) = poreVolm2achieved./rock.poro(fracture_cells);
    ...
    tol=1e-5;
    leftFace=find(abs(G.faces.centroids(:,1)-0)<tol);
    RightFace=find(abs(G.faces.centroids(:,1)-xmaxFS(1))<tol);
    
    FrontFace=find(abs(G.faces.centroids(:,2)-0)<tol);
    BackFace=find(abs(G.faces.centroids(:,2)-xmaxFS(2))<tol);
    
    bcFace=[leftFace;RightFace;FrontFace;BackFace];
    
    bc = addBC([], bcFace, 'flux', 0);
    bc = addThermalBCProps(bc, 'T', repmat(Tinit,[size(bcFace),1]));
    %     schedule = schedule;
