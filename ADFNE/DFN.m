...
%----polygons
    ...
    if(size(opt.polyDomain,1)>0)                                                % clipped by region of study
        out.Poly = PolyClip(plys,opt.polyDomain);
    else
        out.Poly = Clip(plys,opt.bbx);
    end
    length_poly = Length(out.Poly);                                             % Remove fracture where size < minl
    out.Poly = out.Poly(length_poly>0.9*opt.minl,:);
    if Report
